...
describe.each([
	...
	it(`can read from .env with --dotenv: ${cmd}`, async () => {
		...
		await helper.seed({
			...
			"index.ts": dedent`
					export default {
						fetch(request) {
							return new Response(process.env.FOO + ' ' + import.meta.env.BAR);
						}
					}
			`,
		});
		...
		await expect(fetchText(url)).resolves.toBe("some-value some-other-value");
	});
	...
	it(`can set values from --penv: ${cmd}`, async () => {
		...
		const worker = helper.runLongLived(
			`${cmd} --penv FOO=some-value BAR=some-other-value`
		);
